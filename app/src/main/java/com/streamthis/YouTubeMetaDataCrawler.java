...
public class  YouTubeMetaDataCrawler {
    ...
    public static String removeUTFCharacters(String data){
        ...
        while (m.find()) {
            String ch = String.valueOf((char) parseInt(m.group(1), 16));
    ...
    public String getPeopleSubscribed(Document doc)
    {
        return String.valueOf(doc.select(".yt-subscriber-count").text().replace(",", ""));
    }

    public int getPeopleLiked(Document doc)
    {
        return parseInt(doc.select("button.like-button-renderer-like-button-unclicked span").text().replace(",", ""));
    }

    public int getPeopleDisliked(Document doc)
    {
        return parseInt(doc.select("button.like-button-renderer-dislike-button-unclicked span").text().replace(",", ""));
    }
